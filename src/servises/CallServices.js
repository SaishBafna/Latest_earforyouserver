...
const initiateCall = async (callerId, receiverId) => {
    ...
    if (activeCalls[receiverId]) {
        ...
        return { success: false, message: 'User is busy in another call' };
    }
...
const rejectCall = async (receiverId, callerId) => {
    ...
    return { success: true, message: 'Call rejected or marked as missed' };
};
...
const endCall = async (callerId, receiverId) => {
    if (activeCalls[callerId] === receiverId) {
        ...
        return { success: true, message: 'Call ended', duration };
    } else {
...
const handleMissedCall = async (callerId, receiverId) => {
    ...
    return { success: true, message: 'Missed call logged and notification sent' };
};
